...
class Wtf_Fu_Options_Admin {
    ...
    public static function add_new_demo_workflow() {
        ...
        $stages[0]['content_area'] = "<p>The Wtf-Fu plugin provides step wise workflow, so it makes sense for us to use it here to showcase the plugins features, and to provide stepwise instruction on how to use it.</p>
...
<p>Also of note we have turned off the the next button javascript pop up by setting the <code>next_js</code> field for the following stages to an empty string.</p>';
        ...
        $stages[4]['content_area'] = '<p>Below are the file thumnails with the gallery and without reodering enabled.</p>'
                . '<p>The full short code to achieve the action below is :</p>'
        ...
        $stages[5]['content_area'] = "This will then allow users to drag and drop the thumbnail images into their preferred order.</p>"
        ...
        $stages[6]['content_area'] = "<p>You simply create a function in your themes function.php file or inside a file in the mu_plugins directory and then specify the function name (without parenthesis) in either the pre_hook or post_hook field.</p>"
        ...
        $stages[7]['next_js'] = '';
        $stages[7]['content_area'] = "<p>If not please go back and review the steps to make sure that the function is available in either the mu-plugins directory or as part of your themes functions.php file.</p>"
        ...
        $stages[8]['content_area'] = "<p>Normally at this point in a workflow, after you have processed a users uploads etc, you may decide not to allow the user to go back to previous stages any more.</p>"
